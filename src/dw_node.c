    ...
    while (running) {
        ...
        for (int i = 0; i < nfds; i++) {
            ...
            if (type == LISTEN) { // New connection (TCP)
                ...
                check(infos->storage_path && fd == infos->store_replyfd);
...
int main(int argc, char *argv[]) {
    ...
    if (input_args.storage_path) {
        int flags = O_RDWR | O_CREAT | O_TRUNC;
        if (storage_info.use_odirect) flags |= O_DIRECT;
        sys_check(storage_info.storage_fd = open(input_args.storage_path, flags, S_IRUSR | S_IWUSR));
        struct stat s;
        ...
        for (int i = 0; i < input_args.num_threads; i++) {
            thread_infos[i].storage_path = input_args.storage_path;
