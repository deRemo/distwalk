    ...
    while (running) {
        ...
        while (dw_poll_next(&infos->dw_poll, &pflags, &aux)) {
            ...
            if (event_type == LISTEN) { // New connection (TCP)
                ...
                int next_id;
                if (accept_mode == AM_PARENT) { // NOTE: in AM_PARENT mode, only the parent worker thread will reach here
                    next_id = atomic_fetch_add(&next_thread_cnt, 1) % conn_threads;
                    safe_write(conn_worker_infos[next_id].dispatchfd[0], (unsigned char*) &conn_id, sizeof(conn_id));
                } else {
                    next_id = infos->worker_id;
                    if (dw_poll_add(&infos->dw_poll, conn_sock, DW_POLLIN, i2l(SOCKET, conn_id)) != 0)
                ...
                int conn_id;
                if (safe_read(event_data, (unsigned char*) &conn_id, sizeof(conn_id)) < 0) {
                    perror("sock hand-off read() failed");
                    continue;
                }
...
void init_listen_sock(int i, accept_mode_t accept_mode, proto_t proto, struct sockaddr_in serverAddr) {
    ...
    if (accept_mode == AM_PARENT) {
        sys_check(socketpair(AF_UNIX, SOCK_STREAM, 0, conn_worker_infos[i].dispatchfd));
